...
feature_combs = [
    ['uniport_ppi_2019','ppi_2019_dw_40','uniport_bio','uniport_seq','uniport_esm','diffusion_2019'],
    ['uniport_ppi_2019','ppi_2019_dw_40','diffusion_2019']
]

for feature_list in feature_combs:
    name_list = feature_list

    selected_merged_df = merged_df[[col for col in merged_df.columns if any(item in col for item in name_list)]]
    ...
    feature_names, K_path = compute_kernels(X_concat, 'early_all', early_dir, False)
