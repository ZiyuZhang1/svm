...
def evaluate_disease(disease, time, feature_list, df, y, edge_index, methods, time_split):
    ...
    if 'gcn' in methods:
        ...
        result_df.loc[len(result_df.index)] = [feature_list, 1, 'gcn-0-0-0', *results]
        predcition_collection['gcn'+feature_list] = final_y_score
    # if 'gat' in methods:
    ...
    if 'sage' in methods:
        ...
        ranked_predict_index, results = eval_bagging(final_y_score, y_test)
        result_df.loc[len(result_df.index)] = [feature_list, 1, 'sage-0-0-0', *results]
        predcition_collection['sage'+feature_list] = final_y_score
...
def main():
    ...
    if test_bug:
        ...
        out_path = os.path.join(root, 'results/2019_gnn_less_changes')
    ...
    for disease in selected_diseases[:10]:
        ...
        cols_ids = [c for c in df.columns if 'feature' not in c]

        result_list = []
        prediction_collection_all = {}
        for feature_i in feature_list:
            cols = [c for c in df.columns if c.startswith(feature_i)]
            cols.extend(cols_ids)
            feature_df = df[cols].copy()
            print(feature_i, feature_df.shape)
        break
...
#             prediction_collection_all.update({k: v for k, v in predcition_collection.items() if k not in prediction_collection_all})
# #################### save not rewrite
#         result_df_all = pd.concat(result_list, ignore_index=True)
#         result_df_all.to_csv(os.path.join(out_path, f'{disease}.csv'), index=False)
#         with open(os.path.join(out_path_pred, f'{disease}_pred.pkl'), 'wb') as f:
#             pickle.dump(prediction_collection_all, f)

#         mean_df = result_df_all.groupby(['method'])[METRIC_COLUMNS].mean().reset_index()
