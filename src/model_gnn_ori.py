...
def neg_bagging_sage(args):
    ...
    for train_split, val_split, train_split_labels, val_split_labels in splits:
        ...
        model = SimpleGraphSAGE(x.size(1), hidden_channels=in_hidden_channel, dropout=in_drop_out).to(device)
        # optimizer = optim.Adam(model.parameters(), lr=0.01, weight_decay=5e-4)
        optimizer = optim.Adam(model.parameters(), lr=in_lr, weight_decay=in_weight_decay)
        # optimizer = optim.Adam(model.parameters(), lr=0.001, weight_decay=5e-4)
        ...
        for epoch in range(max_epochs):
            ...
            if patience_counter >= patience:
                print(f"Early stopping triggered at epoch {epoch} (best val AUC = {best_split_auc:.4f})")
                break
            # Search "validation auc" for evaluation
        if best_state is None:
            best_state = copy.deepcopy(model.state_dict())
